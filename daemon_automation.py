...
class DaemonAutomation:
    ...
    def run(self):
        ...
        while True:
            ...
            time.sleep(60)  # Vérifier toutes les 60 secondes si une tâche doit s'exécuter
    ...
    def verifier_actions(self):
        ...
        # 1. Vérifier si sourcing à faire
        self.verifier_sourcing()
        
        # 2. Vérifier si clôture à faire
        self.verifier_cloture()
        
        # 3. Vérifier si calcul à faire
    ...
    def verifier_sourcing(self):
        """
        Vérifie si le sourcing doit être lancé pour une journée
        """
        ...
        try:
            ...
            # Chercher les journées à venir qui n'ont pas eu de sourcing
            cursor.execute("""
            ...
            for row in rows:
                ...
                # Calculer quand faire le sourcing
                if type_cal == "serre" or (delai and delai < 7):
                    # Calendrier serré : sourcing dès que possible
                    # On vérifie si la journée précédente est terminée
                    cursor.execute("""
                    ...
                    if precedente and precedente[0] == "terminee":
                        print(f"  🔥 Calendrier serré détecté - Semaine {semaine}")
                        print(f"  🚀 Lancement du sourcing immédiat")
                        self.executer_sourcing(semaine)
                        return
                
                else:
                    # Calendrier normal : sourcing 7 jours avant
                    jours_avant = (date_match - now).days
                    
                    if 6 <= jours_avant <= 8:  # Fenêtre de 7 jours +/- 1 jour
                        print(f"  📅 Semaine {semaine} - J-{jours_avant}")
                        print(f"  🚀 Lancement du sourcing")
                        self.executer_sourcing(semaine)
                        return
    ...
    def executer_sourcing(self, semaine):
        """
        Execute le sourcing pour une semaine
        """
        try:
            bot = SourcingBot()
            bot.run(semaine=semaine)
            
            # Marquer le sourcing comme effectué
            conn = self.db.create_connection()
            cursor = conn.cursor()
            cursor.execute("""
            ...
            # Envoyer notification de nouvelle journée disponible
    ...
    def envoyer_notification_sourcing(self, semaine):
        """
        Envoie un email pour prévenir que la nouvelle journée est disponible
        """
        ...
        try:
            ...
            # Récupérer les infos de la journée
            cursor.execute("""
    ...
    def creer_email_sourcing(self, joueur, semaine, date_match, date_cloture, type_cal):
        """
        Template email nouvelle journée disponible
        """
        alerte = ""
        if type_cal == "serre":
            alerte = """
    ...
    def verifier_cloture(self):
        """
        Vérifie si une clôture doit être effectuée
        """
        ...
        try:
            ...
            # Chercher les journées dont la clôture approche (dans les 5 minutes)
            cursor.execute("""
            ...
            if row:
                ...
                # Si on est à l'heure de clôture (+-5 minutes)
    ...
    def verifier_calcul(self):
        """
        Vérifie si le calcul des gains doit être lancé
        """
        ...
        try:
            ...
            # Chercher les journées en cours dont tous les matchs sont terminés
            cursor.execute("""
            ...
            for row in rows:
                ...
                # Si le dernier match est passé depuis au moins 3h
                heures_apres = (now - date_dernier).total_seconds() / 3600
                
                if heures_apres >= 3:
                    # Vérifier que tous les matchs ont un résultat
                    cursor.execute("""
                    ...
                    print(f"  ℹ️ Attente fin des matchs (dans {int(3-heures_apres)}h)")
    ...
    def executer_calcul(self, semaine):
        """
        Execute le calcul complet : gains + trophées + emails
        """
        try:
            # 1. Calcul des gains
